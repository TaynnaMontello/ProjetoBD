...
app = Flask(__name__)
app.secret_key = "MEGA_SUPER_DIFICIL" 
...
def registrar_log_acesso(usuario_id):
    ip_origem = request.headers.get(
        'X-Forwarded-For',
        request.remote_addr
    )
    ...
    cursor.execute("""
        INSERT INTO log_acesso (usuario_id, ip_origem)
        VALUES (%s, %s)
    """, (usuario_id, ip_origem))
...
def adicionar_livro():
    ...
    if request.method == 'POST':
        ...
        try: 
            ...
            """, (titulo, autor_id, isbn, ano, genero_id, editora_id, qtd, resumo, nome_capa))
            conn.commit()
